...
def get_dataset(dataset: str, path_to_data: str):
    """
    Загружает подготовленные CSV-файлы датасета, кодирует идентификаторы
    пользователей и товаров в диапазон [0, N-1], формирует показатели
    популярности товаров и возвращает готовые структуры данных для обучения моделей.

    Параметры
    ---------
    dataset : str
        Имя набора данных. Допустимые значения:
    ...
    path_to_data : str
        Путь до директории `data`, внутри которой находятся папки
        `raw/` и `preprocessed/`.
    ...
    num_users : int
        Количество уникальных пользователей после перекодировки.

    num_items : int
        Количество уникальных товаров после перекодировки.

    item_popularity : np.ndarray
        Вектор популярности товаров, рассчитанный на основе частоты
        появления товара в положительных взаимодействиях.
    """
